...
for index, row in tabela.iterrows():
  ...
  # Cláusula Quarta
    ...
    c44 = c4.add_run(f'\nA despesa decorrente do presente TERMO correrá pelo ')
    c44.font.size = Pt(11)
    c44.font.name = 'Arial'
    c44.font.size = Pt(11)
    c44.font.name = 'Arial'
    ...
    doc.add_page_break()
    #iterando sobre os empenhos
    for index, row in tabela.iterrows():
      dados_filtrados = dados_despesas[dados_despesas['PROCESSOS'] == processo]
    if not dados_filtrados.empty:  # Verifica se o DataFrame não está vazio
      qtd_dados = len(dados_filtrados)
...
for index, row in instruir.iterrows():
  ...
  # Informaões do Termo
    ...
    valor_monetario_por_extenso = valor_monetario_por_extenso(valor_numerico)
    valor_monetario_por_extenso = ' (' + valor_monetario_por_extenso + ')'
    #python reconhece o número 14 como "Catorze" no valor monetário por extenso
  ...
  # Notas Fiscais
    if processo[-4:] == "2025":
      # Obter todos os dados da planilha como strings usando get_all_values()
      notas = gc.open('Dados TAC').worksheet('Notas 2025').get_all_values()
      notas = pd.DataFrame(notas[1:], columns=notas[0])
      notas['NOTAS'] = notas['NOTAS'].astype(str)
    else:
      notas = gc.open('Dados TAC').worksheet('Notas 2024').get_all_values()
      notas = pd.DataFrame(notas[1:], columns=notas[0])
      notas['NOTAS'] = notas['NOTAS'].astype(str)
  ...
  # Informações
    ...
    periodo_inicial = row["PERÍODO INICIAL"]
    periodo_final = row["PERÍODO FINAL"]
    #para verificar se o documento é termo de ajustes de contas ou termo de reconhecimento de divídas
    ano_atual = str(datetime.now().year)

    ano_referencia = periodo_inicial.split('/')
  ...
  # Despacho
    ...
    if certidão == "IRREGULAR":
      ...
      paragrafo1 = paragrafo.add_run(f'        Encaminho o presente processo administrativo para emissão de parecer jurídico quanto à possibilidade de pagamento através de Ajuste de Contas/Reconhecimento de Dívida e CND irregular (fl. {folha}), com base nos documentos encartados aos autos.')
      paragrafo1.font.size = Pt(fonte_padrão)
      paragrafo1.font.name = 'Arial'
  ...
  # Nome do arquivo
    processo_format = processo.replace('/','.')

    end_time = time.time()

    # Salvamento
    caminho = f'/content/drive/MyDrive/TERMOS E AUTORIZOS/{ano}/{empresa}'
    caminho_format = f'{caminho}/{processo_format}.docx'
    lista_processos.append(caminho_format)
    caminho_copia = f'/content/drive/MyDrive/TERMOS E AUTORIZOS/PROCESSOS PARA IMPRESSÃO.docx'
    ...
    doc.save(caminho_format)
    print(f"Autorizo para processo nº {processo} gerado com sucesso!")

doc_impress = Document('/content/drive/MyDrive/docx_exemplo.docx')

for lista in lista_processos:
    doc = Document(lista)
    for element in doc.element.body:
        doc_impress.element.body.append(element)

# Salvar o documento combinado
doc_impress.save(caminho_copia)
  ...
  # Informaões
    ...
    if gerar_doc == '':
      ...
      if documentos == "TERMO E AUTORIZO":
        folha = int(folha) + 1
        print(f'processando termo para o processo {processo}')
      ...
      # AUTORIZO
        folha = folha + 1
        print(f'processando autorizo para o processo {processo}')
        num_processo = doc.add_paragraph('\n')
        num_processo.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        paragraph_format = num_processo.paragraph_format
        paragraph_format.line_spacing_rule = WD_LINE_SPACING.SINGLE
      ...
      # Secretaria
        ...
        doc.save(caminho)
        print(f"Termo e Autorizo para processo nº {processo} gerado com sucesso!")
        print('--------------------------------------------------------------')
...
def add_page_numbers_to_front(pdf_path, output_path):
    # Abrir o PDF original
    reader = PdfReader(pdf_path)
    writer = PdfWriter()

    # Criar um objeto PDF para cada página ímpar
    for page_num in range(len(reader.pages)):
        if page_num % 2 == 0:  # Página ímpar (índices baseados em zero)
            # Criar uma nova página com numeração
            packet = io.BytesIO()
            can = canvas.Canvas(packet, pagesize=letter)

            # Adicionar o número da página (baseado no índice da página)
            page_number = page_num // 2 + 1
            can.drawString(500, 50, str(page_number))  # Adiciona o número no canto inferior direito
            can.save()

            # Ler o PDF gerado e mesclar com a página original
            packet.seek(0)
            new_pdf = PdfReader(packet)
            page = reader.pages[page_num]
            page.merge_page(new_pdf.pages[0])  # Mescla a numeração com a página

        else:
            page = reader.pages[page_num]  # Páginas pares não recebem numeração

        writer.add_page(page)

    # Salvar o PDF com numeração
    with open(output_path, "wb") as output_file:
        writer.write(output_file)

# Exemplo de uso
input_pdf = "/content/GUIA DE FGTS.pdf"  # Substitua pelo caminho do seu PDF
output_pdf = "arquivo_numerado.pdf"
add_page_numbers_to_front(input_pdf, output_pdf)
...
# Obter os processos da planilha de filtros como uma lista
for index, row in tabela.iterrows():
  processos_filtro = row['Processo']
  dados_filtrados = dados_despesas[dados_despesas['PROCESSOS'] == processos_filtro]
  if not dados_filtrados.empty:  # Verifica se o DataFrame não está vazio
    qtd_dados = len(dados_filtrados)
    print(qtd_dados)
    pen = len(dados_filtrados) - 1
    print('A despesa decorrente do presente TERMO correrá pelo ')
    if qtd_dados == 1:
      for index, row in dados_filtrados.iterrows():
        empenho = row['EMPENHO']
        programa_trabalho = row['PROGRAMA DE TRABALHO']
        ...
        codigo_despesa = row['CÓDIGO DE DESPESA']
        fonte = row['FONTE']
        print(f'empenho nº {empenho}/2025 à Conta do Programa de Trabalho nº {programa_trabalho}, Código de Despesa nº {codigo_despesa} fonte {fonte}.')
      ...
      for index, row in dados_filtrados.iterrows():
        ...
        if num_dados == pen:
          ...
          preposicao = '.'
        print(f'empenho nº {empenho}/2025 à Conta do Programa de Trabalho nº {programa_trabalho}, Código de Despesa nº {codigo_despesa} fonte {fonte}{preposicao}')
        num_dados = num_dados + 1
...
for index, row in tabela.iterrows():
  ...
  # Cláusula Quarta
    ...
    if not dados_filtrados.empty:  # Verifica se o DataFrame não está vazio
      ...
      if qtd_dados == 1:
      ...
      elif qtd_dados > 2 and qtd_dados <= 3:
        num_dados = 1
        for index, row in dados_filtrados.iterrows():
          processo = row['PROCESSOS']
          empenho = row['EMPENHO']
        ...
        num_dados = 1
        # Itera apenas sobre as 5 primeiras linhas
        for i in range(min(3, len(dados_filtrados))):
          row = dados_filtrados.iloc[i]
          processo = row['PROCESSOS']
          empenho = row['EMPENHO']
          programa_trabalho = row['PROGRAMA DE TRABALHO']
          valor = row['VALOR']
